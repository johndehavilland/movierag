...
public class CosmosDbService
{
    ...
    /// <summary>
    /// Gets a list of all movies where embeddings are null.
    /// </summary>
    public async Task<List<Movie>> GetMoviesToVectorizeAsync()
    {
        QueryDefinition query = new QueryDefinition("SELECT * FROM c WHERE IS_ARRAY(c.embedding)=false");

        FeedIterator<Movie> results = _container.GetItemQueryIterator<Movie>(query);

        List<Movie> output = new();
        while (results.HasMoreResults)
        {
            FeedResponse<Movie> response = await results.ReadNextAsync();
    ...
    /// <summary>
    /// Gets a count of all movies based on embeddings status.
    /// </summary>
    public async Task<int> GetMovieCountAsync(bool withEmbedding)
    {
    ...
    {
        BulkOperations<Movie> bulkOperations = new BulkOperations<Movie>(movies.Count);
        foreach (Movie movie in movies)
        {
            bulkOperations.Tasks.Add(CaptureOperationResponse(_container.CreateItemAsync(movie, new PartitionKey(movie.id)), movie));
        }
        BulkOperationResponse<Movie> bulkOperationResponse = await bulkOperations.ExecuteAsync();
    }
    ...
    public async Task UpdateMoviesAsync(List<string> docsToUpdate)
    {
        BulkOperations<Movie> bulkOperations = new BulkOperations<Movie>(docsToUpdate.Count);
        foreach ( string entry in docsToUpdate) 
        {
            await _container.PatchItemAsync<Movie>(entry,new PartitionKey(entry), patchOperations: new[] { PatchOperation.Add("/embeddingStatus", true)});
        }
    }
